...
| 11.9 | Removal is mask-only | After a mask application, inspect the Removal Region and expand the generated address review. Try to find any Scene View picking, numeric address, or submesh bulk-delete control. | The generated selection is reviewable but read-only; the only operation that changes it is applying the texture mask with Replace, Add To Selection, or Subtract From Selection. No manual triangle or address-edit control is present. | |
