...
**当前版本是发布候选 `0.3.0-rc.12`（见 `package.json`）。它不是 1.0，完整验收清单仍未全部完成。**

当前接缝候选色使用 `#RRGGBB` 色号输入（也接受 `#RRGGBBAA`），输入无效时保留上一次有效颜色；默认值为
`#000000`。移除区域只通过黑白遮罩生成，Scene View 拾取、数字地址和子网格批量删除已移除，生成的地址列表仅供只读检查。
...
`0.3.0-rc.12` 默认启用 Play Mode 兼容层。只要已加载场景中存在 `AvatarPartInstaller`，APA 会在进入
...
自动生成现在只在**部件网格**上读取顶点色：窗口中的 `Candidate Color` 默认是黑色 `#000000`，只有部件 `Mesh.colors32`
的四个通道（含 alpha）与所选颜色**完全相等**的顶点才是候选。目标网格不需要顶点色，目标侧仍按世界空间位置搜索。
部件缺失顶点色、颜色数量与顶点数不一致、或没有顶点携带所选颜色，都会以 `APA052 SEAM_CANDIDATE_COLOR_INVALID`
阻断，绝不会退回全网格搜索。旧的 `ApaMergeVertexGroup` 组件与同名
`merge vertex` 骨骼表示已经移除；`APA051` 保留原义并退役，不会复用于新条件。绿色候选叠加层显示部件候选顶点，
`Merge Check Overlay` 会暂时隐藏普通叠加层，把预计匹配的目标和部件顶点画成绿色、没有匹配的候选画成红色，且不写入
Profile。色轮拖动期间会延迟读取候选和重跑合并检查，避免编辑器卡顿。重新打开窗口或加载已有 Profile 时，目标骨架和部件
骨架会从 Profile 保存的相对路径恢复。红色移除叠加层默认关闭，可单独打开；打开后它的悬停与点击仍跟随目标当前形态键姿势，
而接缝配对与最终构建继续使用静置姿势。
