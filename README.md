...
当前版本：`0.5.2` · Unity `2022.3` · 界面支持 English / 简体中文
...
- 受保护部件与普通部件共用同一条读取路径：载荷按内容身份解码一次并复用；未受保护的部件永远不会进入编解码器。
- 预览的几何体可以保持缓存，材质列表则在每一帧从当前渲染器的 `sharedMaterials` 与 Profile 的材质声明重新解析，
  因此替换材质槽或修改材质资产都会立刻反映在预览中，并且不会克隆材质。
- Profile 里的材质语义行只决定语义名、来源子网格与冲突策略；该槽位的实际材质取部件渲染器当前的
  `sharedMaterials[来源子网格]`，只有渲染器没有该槽位或该槽位没有材质时才回退到 Profile 中记录的资产。
  安装者在渲染器（含预制件实例）上替换的材质因此会成为构建实际使用的材质，且不会被写回 Profile。
...
11. 使用受保护网格模式时，预制体与其 `_ProtectedMesh.asset` 必须一起分发；不要手工编辑载荷资产的字节，
    也不要在源网格改变后继续使用旧的受保护预制体。
